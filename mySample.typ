...
游戏使用C\#实现了如下系统
1. GamaManager，线程安全的单例模式。游戏加载时创建并且保证只有一份实例，全局可访问。保存玩家数据，事件库，UI对象的引用，游戏状态等。
2. UIDisplay类，所有UIDisplay的基类，包括了所有UIDisplay的基本操作，如显示，隐藏，更新等。其他继承自该父类的基类会使用Unity提供的可视化剪辑器将子类UIDisplay（例如Text）绑定到UI类的属性上。
