...
我在华中科技大学国家战略传播实验室实习，主要是完成一个模拟系统的demo搭建工作。具体来说我的工作包括UI搭建，数据加载，逻辑实现，以及系统的整体架构设计。技术栈用的Unity&&C\#，写的很爽。
...
#img(
  image("assets/codeRight.png"),
  caption: "正确代码"
)
目的：定义匿名函数，在按钮被点击时候能够调用匿名函数。但是在开发过程中遇到了需要更多信息来判断按钮是哪一个的问题。因为不同的按钮应该通过相同的匿名函数调用不同的影响事件。
...
异步GUI阻塞问题：
  在自定义事件系统中，玩家可以通过在GUI里面输入文本内容和数值自定义事件。但是系统又需要玩家随时点击关闭按钮结束这个面板，这要求IO操作和面板的操作避免同步关系

  使用async/await解决拥塞问题。在使用asnyc标记的函数里面可以使用 await标记那些会导致拥塞的IO操作，使得cpu在执行到标记点出会自动展开一个task程序状态机，当状态机发生变化线程才会重新获得cpu控制权。避免了cpu空转拥塞等待IO操作完成而没有办法执行其他UI操作逻辑
